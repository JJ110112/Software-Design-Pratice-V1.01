const fs = require('fs');
const fn = 'c:/Users/hitea/Antigravity/Software Design Pratice/js/api.js';
let content = fs.readFileSync(fn, 'utf-8');

const regex = /let app, db;[\s\S]*/;

const newCode = \`let app, db;
...
window.saveScore = async function (className, userName, qID, gameMode, timeSpent, status = "PASS", stars = 3) {
    ...
    try {
        const newRecord = {
            className, userName, qID, gameMode, timeSpent, status, stars,
