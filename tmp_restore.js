...
const regexToFix = /function finishGame\(\) \{[\s\S]*?setupNextLevelButton\(\);\r?\n\}/;

const newFn = `function finishGame() {
  isFinished = true;
  clearInterval(timer);
  fillEl.style.width = '100%';
  document.getElementById('percent-label').textContent = '100%';
  playWin();
  ...
  if (typeof getCurrentUser === 'function') {
      ...
      if (user && typeof window.saveScore === 'function') {
          window.saveScore(user.className, user.name, qID + '_' + tID, "看中文寫程式", seconds, "PASS", stars)
...
content = content.replace(regexToFix, newFn);
